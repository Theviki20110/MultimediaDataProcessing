...
std::ofstream& raw_write(std::ofstream& os, uint8_t sym, uint8_t len, uint32_t code) {
	os.write(reinterpret_cast<char*>(&sym), sizeof(sym));
...
int main() 
{
	...
	for (const auto& elem : huff.table())
		raw_write(os, elem._sym, elem._len, elem._code);
