...
elem hufstr::find(uint8_t sym) const {
	elem tmp;
	for (const auto& x : _huff_table) {
		if (x._sym == sym) {
			tmp = x;
			break;
		}
...
uint8_t hufstr::reverse_find(uint32_t code, uint8_t len) const {
	uint8_t out = 0;

	for (const auto& x : _huff_table) {
		if ((x._code == code) && (x._len == len)) {
			out = x._sym;
			break;
		}
	}
...
std::vector<uint8_t> hufstr::compress(const std::string& s) const {
	...
	for (const auto& x : s) {
		...
		for (uint8_t i = 0; i < tmp._len; i++) {
			...
			buffer = (buffer << 1) | tmp._code >> ((tmp._len - 1) - i);
			n--;
		}
	}
...
std::string hufstr::decompress(const std::vector<uint8_t>& v) const {
	...
	for (const auto& x : v) {
		...
		for (size_t i = 8; i > 0; i--) {
			sym = reverse_find(buffer, len);
			if (sym) {
				out.push_back(sym);
				buffer = 0;
				len = 0;
			}
			buffer = (buffer << 1) | (x >> (i - 1) & 1);
			len++;
		}
...
/*int main() {
	...
	vector<uint8_t> out = huffstr.compress("aeio");
	cout << huffstr.decompress(out);
